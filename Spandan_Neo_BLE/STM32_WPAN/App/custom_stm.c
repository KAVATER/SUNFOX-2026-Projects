...
extern unsigned char BLE_RX_BUF[];
extern void BLE_Receive_Handler(void);
//int Data_Sending_Enable = 0;
/* USER CODE END Includes */

/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomSpandan_PatchHdle;                    /**< SPANDAN_PATCH handle */
  uint16_t  CustomChar_WrHdle;                  /**< CHAR_WR handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeChar_Wr = 1;
uint16_t SizeMycharnotify = 120;
...
#define COPY_SPANDAN_PATCH_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_CHAR_WR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomChar_WrHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
            /* USER CODE BEGIN CUSTOM_STM_Service_1_Char_1_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */
			//CDC_Transmit_FS(attribute_modified->Attr_Data,attribute_modified->Attr_Data_Length);

            /* UTSAV RX ROUTINE write the receive routine here */

            memcpy(BLE_RX_BUF,attribute_modified->Attr_Data,attribute_modified->Attr_Data_Length);

            BLE_Receive_Handler();

            /* USER CODE END CUSTOM_STM_Service_1_Char_1_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */
          } /* if (attribute_modified->Attr_Handle == (CustomContext.CustomChar_WrHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for SPANDAN_PATCH +
   *                                2 for CHAR_WR +
   *                                2 for myCharNotify +
  ...
  COPY_SPANDAN_PATCH_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomSpandan_PatchHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: SPANDAN_PATCH, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: SPANDAN_PATCH \n\r");
  }

  /**
   *  CHAR_WR
   */
  COPY_CHAR_WR_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomSpandan_PatchHdle,
                          UUID_TYPE_128, &uuid,
                          SizeChar_Wr,
                          CHAR_PROP_WRITE,
                          ...
                          &(CustomContext.CustomChar_WrHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : CHAR_WR, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : CHAR_WR \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHAR_WR:
      ret = aci_gatt_update_char_value(CustomContext.CustomSpandan_PatchHdle,
                                       CustomContext.CustomChar_WrHdle,
                                       0, /* charValOffset */
                                       SizeChar_Wr, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value CHAR_WR command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value CHAR_WR command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHAR_WR:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomSpandan_PatchHdle, CustomContext.CustomChar_WrHdle, SizeChar_Wr, pPayload);
    ...
    case CUSTOM_STM_MYCHARNOTIFY:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomSpandan_PatchHdle, CustomContext.CustomMycharnotifyHdle, SizeMycharnotify, pPayload);
