...
{
	uint32_t bytesToSend = size * 2;
	uint32_t BufferSize = (bytesToSend < BURST_MAX_SIZE) ? bytesToSend : BURST_MAX_SIZE;
	if(BufferSize == 0)
	{
		return;
	}
	...
	{
		HAL_SPI_Transmit(HSPI_INSTANCE, BurstBuffer, BufferSize, 100);
		remaining -= BufferSize;
	}

	if(remaining > 0)
	{
		HAL_SPI_Transmit(HSPI_INSTANCE, BurstBuffer, remaining, 100);
	}
	HAL_GPIO_WritePin(LCD_CS_PORT, LCD_CS_PIN, GPIO_PIN_SET);
}
...
void ILI9341_DrawPixel(uint16_t x,uint16_t y,uint16_t color)
{
	...
	uint8_t bufferX[4] = {x>>8, x, x>>8, x};
	uint8_t bufferY[4] = {y>>8, y, y>>8, y};
