...
#define Max_Connections CFG_BLE_NUM_LINK
typedef struct
{
...
connection_context_t connection[CFG_BLE_NUM_LINK];	
...
typedef struct _tBLEProfileGlobalContext
{
  ...
  //comment the following defination if cubeMx regenerate it
  uint16_t connectionHandle;
...
uint8_t a_AdvData[9] =
{
  8, AD_TYPE_COMPLETE_LOCAL_NAME, 'S', 'T', 'M', '_', 'B', 'L', 'E',  /* Complete name */
...
void APP_BLE_Init(void)
{
  ...
  {
	  connection[i].connectionHandle = 0xFFFF;
...
//      }
      ...
      //the following code is used to free the connection handle of the disconnected device
            for(int i = 0; i<Max_Connections; i++)
            {
          	  if(connection[i].connectionHandle == p_disconnection_complete_event->Connection_Handle)
          	      {
          	          connection[i].connectionHandle=0xFFFF;//resetting the disconnected slot
          	          connection[i].connected=0;
      ...
      if(connected_Devices < CFG_BLE_NUM_LINK)
      {
          Adv_Request(APP_BLE_FAST_ADV);
      }
...
//----------------If cubeMx uncomment the following code then comment if back as it contradicts the above logic
...
//------------ connected --------------//
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          //comment back the following if cubeMx regenerates it as this line of code preassumes only one connection exists
          //BleApplicationContext.BleApplicationContext_legacy.connectionHandle = p_connection_complete_event->Connection_Handle;
          for(int i = 0; i< CFG_BLE_NUM_LINK; i++)
          {
        	  if(connection[i].connected == 0)
        	  {
        		  connection[i].connectionHandle = p_connection_complete_event->Connection_Handle;
        		  connection[i].connected = 1;//flag showing slot is filled
        		  //the above code is storing the connection handle of the newly connected device into the ith sloth of connection table
        		  break;
        	  }
          }
          /**
          ...
          if(connected_Devices < CFG_BLE_NUM_LINK)
          {
        	  //APP_DBG_MSG("Starting advertising for next central.\n");
        	     Adv_Request(APP_BLE_FAST_ADV);//advertising at fast mode
          }
...
uint8_t APP_BLE_Get_Connected_Device_Count()
{
    return connected_Devices;
}
uint16_t APP_BLE_Get_Connection_Handle( uint8_t index)
{
    return connection[index].connectionHandle;
}
