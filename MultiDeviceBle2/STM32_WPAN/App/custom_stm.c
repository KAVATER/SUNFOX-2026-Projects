...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomServicebleHdle;                    /**< ServiceBLE handle */
  uint16_t  CustomCharnotifyHdle;                  /**< CharNotify handle */
...
#define COPY_SERVICEBLE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_CHARNOTIFY_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for ServiceBLE +
   *                                2 for CharNotify +
   *                                1 for CharNotify configuration descriptor +
   *                              = 4
   *
  ...
  COPY_SERVICEBLE_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomServicebleHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: ServiceBLE, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: ServiceBLE , handle = 0x%04x \n\r", CustomContext.CustomServicebleHdle);
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomServicebleHdle,
                          UUID_TYPE_128, &uuid,
                          SizeCharnotify,
                          CHAR_PROP_NOTIFY,
                          ...
                          &(CustomContext.CustomCharnotifyHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : CHARNOTIFY, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : CHARNOTIFY , handle = 0x%04x \n\r", CustomContext.CustomCharnotifyHdle);
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHARNOTIFY:
      ret = aci_gatt_update_char_value(CustomContext.CustomServicebleHdle,
                                       CustomContext.CustomCharnotifyHdle,
                                       0, /* charValOffset */
                                       SizeCharnotify, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value CHARNOTIFY command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value CHARNOTIFY command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHARNOTIFY:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomServicebleHdle, CustomContext.CustomCharnotifyHdle, SizeCharnotify, pPayload);
