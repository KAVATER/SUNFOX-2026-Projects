...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomStm32BleHdle;                    /**< STM32BLE handle */
  uint16_t  CustomCharbleHdle;                  /**< charBLE handle */
...
#define COPY_STM32BLE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_CHARBLE_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomCharbleHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_CHARBLE_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_CHARBLE_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
...
void SVCCTL_InitCustomSvc(void)
{
  ...
  /* Notify adds the CCCD descriptor, so the service needs one more attribute. */
  max_attr_record = 4;
   ...
   * service_max_attribute_record = 1 for STM32BLE +
   *                                2 for charBLE +
   *                                1 for charBLE configuration descriptor +
   *                              = 4
   *
  ...
  COPY_STM32BLE_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomStm32BleHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: STM32BLE, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: STM32BLE , handle = 0x%04x \n\r", CustomContext.CustomStm32BleHdle);
  }

  /**
   *  charBLE
   */
  COPY_CHARBLE_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomStm32BleHdle,
                          UUID_TYPE_128, &uuid,
                          SizeCharble,
                          CHAR_PROP_NOTIFY,
                          ...
                          &(CustomContext.CustomCharbleHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : CHARBLE, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : CHARBLE , handle = 0x%04x \n\r", CustomContext.CustomCharbleHdle);
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHARBLE:
      ret = aci_gatt_update_char_value(CustomContext.CustomStm32BleHdle,
                                       CustomContext.CustomCharbleHdle,
                                       0, /* charValOffset */
                                       SizeCharble, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value CHARBLE command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value CHARBLE command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHARBLE:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomStm32BleHdle, CustomContext.CustomCharbleHdle, SizeCharble, pPayload);
